...
public class Graph {
    ...
    //associates with each vertex the edges that are incident to it
	HashMap<Vertex, Set<Edge>> vertexEdgesMap = new HashMap<>();
	/**
	...
	public Graph(List<Pair> pairs){
		...
		for(Pair e : pairs){
			...
			//update vertexEdgesMap
			Set<Edge> edges1 = vertexEdgesMap.get(v1);
			if(edges1 == null) {
				edges1 = new HashSet<>();
			}
			edges1.add(edge);
			vertexEdgesMap.put(v1, edges1);
			
			Set<Edge> edges2 = vertexEdgesMap.get(v2);
			if(edges2 == null) {
				edges2 = new HashSet<>();
			}
			edges2.add(edge);
			vertexEdgesMap.put(v2, edges2);		
		}
	}
	...
	public boolean pathExists(Vertex u, Vertex v) {
		...
		while (!queue.isEmpty()) {
			...
			List<Vertex> adjVertices = adjList.get(vertex);
			for (Vertex adjVertex : adjVertices) {
				if (adjVertex.equals(v)) return true;
	...
	public List<Graph> getConnectedComponents() {
		...
		for (Vertex rootVertex : vertices) {
			if (!visited.contains(rootVertex)) {
				queue.add(rootVertex);
				visited.add(rootVertex);
				List<Edge> edgeList = new ArrayList<>(vertexEdgesMap.get(rootVertex));
				while (!queue.isEmpty()) {
					Vertex vertex = queue.poll();
					List<Vertex> adjVertices = adjList.get(vertex);
					for (Vertex adjVertex : adjVertices) {
						if (!visited.contains(adjVertex)) {
							queue.add(adjVertex);
							visited.add(adjVertex);
							edgeList.addAll(vertexEdgesMap.get(adjVertex));
						}
					}
				}
				subGraphs.add(new Graph(edgeList.toArray(value -> new Edge[0])));
			}
		}
		return subGraphs;
	}
		...
		for (Vertex rootVertex : vertices) {
			if (!visited.contains(rootVertex)) {
				...
				while (!queue.isEmpty()) {
					...
					List<Vertex> adjVertices = adjList.get(vertex);
					for (Vertex adjVertex : adjVertices) {
						if (visited.contains(adjVertex)) return false;
						queue.add(adjVertex);
						visited.add(adjVertex);
					}
				}
